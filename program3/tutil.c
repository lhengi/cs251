 ...
 *           -2, -1, 0, 1, 2
 */
void seq_build(int n, int start, BST **pt) {
  int i, x;
    *pt = bst_create();
...
void seq_build_descending(int n, int start, BST **pt) {
  int i, x;
    *pt = bst_create();
    for(i=1, x=start; i<=n; i++, x--)
        bst_insert(*pt, x);
 ...
 */
int rem_range(BST *t, int lo, int hi){
  int x;
  int ndel = 0;
 ...
 */
int rem_first_third(BST *t, int n, int *hi) {
  int third;
...
int rem_last_third(BST *t, int n, int *lo) {

  *lo = (2*n)/3;
  return rem_range(t, *lo, n);
}

int rem_middle_third(BST *t, int n, int *lo, int *hi) {
...
BST * build_balanced_rem_evens(int h) {
...
int leq_testB(int h, int *ntests, int *npassed) {
  ...
  *npassed=0;
  ...
  for(x=1; x<=n; x++) {
    n_odds += (x%2);
    if(bst_num_leq(t, x)==n_odds)
      (*npassed)++;
    (*ntests)++;
  }
  n_evens = (n+1)/2;
  if(bst_num_leq(t, n+2) == n_evens)
    (*npassed)++;
  (*ntests)++;
  bst_free(t);
  return 1;
...
int range_testB(int h) {
  ...
  passed = 1;
  for(i=0; i<n; i++) {
    if(bst_num_range(t, lo, hi) != delta)
      passed = 0;
  }
  bst_free(t);
  return passed;
...
int range_testC(int h) {
  ...
  passed = 1;
  for(i=0; i<n; i++) {
    if(bst_num_range(t, lo, hi) != answer)
      passed = 0;
  }
  bst_free(t);
  return passed;
